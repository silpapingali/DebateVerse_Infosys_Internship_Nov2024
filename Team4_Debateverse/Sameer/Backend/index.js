...
app.post("/reset-password/:token", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Password successfully updated! You can now log in." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
app.get("/debates", middleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Failed to fetch debates." });f
  }
});
...
app.get("/alldebates", middleware, async (req, res) => {
  ...
  try {
    ...
    const totalRecords = await Debate.countDocuments({
      createdBy: { $ne: createdBy },
    });

    // Fetch debates with pagination and sorting
    const debates = await Debate.find({ createdBy: { $ne: createdBy } })
...
app.post("/vote/:id", middleware, async (req, res) => {
  ...
  try {
    ...
    // Update total votes
    debate.totalVotes = debate.options.reduce(
...
app.get("/allusers", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching users and their debates." });
  }
});
...
app.delete("/debate/:debateId/option/:optionId", async (req, res) => {
  ...
  try {
    ...
    const optionToDelete = debate.options.find(
    ...
    debate.options = debate.options.filter(
      (option) => option._id.toString() !== optionId
    );
...
app.patch("/:debateId/unblock", middleware, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Access denied. Only admins can unblock debates." });
    }
